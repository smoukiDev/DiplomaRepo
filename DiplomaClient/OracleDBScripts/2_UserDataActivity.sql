...
create or replace PROCEDURE UNBLOCKUSER
(v_id in CLIENTMODULEACCESS.USERID%TYPE)
IS
BEGIN
UPDATE CLIENTAPPUSERS
...
v_pass in ClientAppUsers.PASS%TYPE,
v_passhash in ClientAppUsers.PASSHASH%TYPE,
v_isblock in ClientAppUsers.IsBlock%TYPE,
v_fname in CLIENTAPPUSERS.FNAME%TYPE,
v_lname in CLIENTAPPUSERS.LNAME%TYPE,
v_mname in CLIENTAPPUSERS.MNAME%TYPE,
v_gender in CLIENTAPPUSERS.GENDER%TYPE,
...
WHERE USERID = v_userid;
END UPDATEUSER;
--CREATE USER UNDER ADMIN PRIVILEDGE
...
VALUES(idbuf,'F','F','F','F','F','F');
END CREATEUSER;
--Run Analysis Monitor
create or replace PROCEDURE RUNANILYZELOG
(
v_id in ClientAppUsers.USERID%TYPE,
v_modelname in CLIENTAPPUSERACTIVITY.DESCRIPTION%TYPE
)
IS
BEGIN
INSERT INTO CLIENTAPPUSERACTIVITY (USERID,DESCRIPTION,ACTIVITYTIME)
VALUES (v_id,'User has run '||v_modelname||' model', CURRENT_DATE);
END RUNANILYZELOG;
--Export Monitor
create or replace PROCEDURE EXPORTLOG
(
v_id in ClientAppUsers.USERID%TYPE,
v_modelname in CLIENTAPPUSERACTIVITY.DESCRIPTION%TYPE
)
...
BEGIN
INSERT INTO CLIENTAPPUSERACTIVITY (USERID,DESCRIPTION,ACTIVITYTIME)
VALUES (v_id,'User has exported '||v_modelname||' model', CURRENT_DATE);
END EXPORTLOG;
--Export Monitor
--!!!Launch printed but not begin
create or replace PROCEDURE PRINTLOG
(
v_id in ClientAppUsers.USERID%TYPE,
v_modelname in CLIENTAPPUSERACTIVITY.DESCRIPTION%TYPE
)
...
BEGIN
INSERT INTO CLIENTAPPUSERACTIVITY (USERID,DESCRIPTION,ACTIVITYTIME)
VALUES (v_id,'User has printed '||v_modelname||' model', CURRENT_DATE);
END PRINTLOG;